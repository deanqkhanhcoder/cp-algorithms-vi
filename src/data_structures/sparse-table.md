...
Nó có thể trả lời hầu hết các truy vấn trên đoạn trong $O(\log n)$, nhưng sức mạnh thực sự của nó là trả lời các truy vấn tìm giá trị nhỏ nhất trên đoạn (hoặc các truy vấn tìm giá trị lớn nhất tương đương).
Đối với các truy vấn đó, nó có thể tính toán câu trả lời trong thời gian $O(1)$.

Nhược điểm duy nhất của cấu trúc dữ liệu này là nó chỉ có thể được sử dụng trên các mảng _bất biến_.
...
Theo cùng một lý luận, bất kỳ khoảng nào cũng có thể được biểu diễn duy nhất dưới dạng hợp của các khoảng có độ dài là các lũy thừa giảm dần của hai.
Ví dụ: $[2, 14] = [2, 9] \cup [10, 13] \cup [14, 14]$, trong đó khoảng hoàn chỉnh có độ dài 13, và các khoảng riêng lẻ có độ dài lần lượt là 8, 4 và 1.
Và ở đây, hợp cũng bao gồm nhiều nhất là $\lceil \log_2(\text{độ dài của khoảng}) \rceil$ khoảng.
...
Sau đó, một truy vấn trên đoạn khác có thể được trả lời bằng cách chia đoạn thành các đoạn có độ dài là lũy thừa của hai, tra cứu các câu trả lời đã được tính toán trước, và kết hợp chúng để nhận được một câu trả lời hoàn chỉnh.
...
$\\text{K}$ phải thỏa mãn $\\text{K} \ge \lfloor \log_2 \text{MAXN} \rfloor$, vì $2^{\lfloor \log_2 \text{MAXN} \rfloor}$ là đoạn có độ dài lũy thừa của hai lớn nhất mà chúng ta phải hỗ trợ.
Đối với các mảng có độ dài hợp lý ($\le 10^7$ phần tử), $K = 25$ là một giá trị tốt.

Chiều $\\text{MAXN}$ đứng thứ hai để cho phép truy cập bộ nhớ liên tiếp (thân thiện với bộ đệm).

```{.cpp file=sparsetable_definition}
int st[K + 1][MAXN];
```

Vì đoạn $[j, j + 2^i - 1]$ có độ dài $2^i$ chia nhỏ thành các đoạn $[j, j + 2^{i - 1} - 1]$ và $[j + 2^{i - 1}, j + 2^i - 1]$, cả hai đều có độ dài $2^{i - 1}$, chúng ta có thể tạo bảng một cách hiệu quả bằng cách sử dụng quy hoạch động:
...
Đối với các truy vấn tổng trên đoạn, nó sẽ tính tổng, đối với các truy vấn tìm giá trị nhỏ nhất trên đoạn, nó sẽ tính giá trị nhỏ nhất.

Độ phức tạp thời gian của việc tiền xử lý là $O(\text{N} \log \text{N})$.
...
Để trả lời truy vấn tổng cho đoạn $[L, R]$, chúng ta lặp qua tất cả các lũy thừa của hai, bắt đầu từ cái lớn nhất.
...
Độ phức tạp thời gian cho một Truy vấn tổng trên đoạn là $O(K) = O(\log \text{MAXN})$.

## Truy vấn tìm giá trị nhỏ nhất trên đoạn (RMQ)

Đây là các truy vấn mà Bảng thưa tỏa sáng.
Khi tính toán giá trị nhỏ nhất của một đoạn, không quan trọng nếu chúng ta xử lý một giá trị trong đoạn một hoặc hai lần.
Do đó, thay vì chia một đoạn thành nhiều đoạn, chúng ta cũng có thể chia đoạn thành chỉ hai đoạn chồng chéo có độ dài là lũy thừa của hai.
Ví dụ: chúng ta có thể chia đoạn $[1, 6]$ thành các đoạn $[1, 4]$ và $[3, 6]$.
Giá trị nhỏ nhất của đoạn $[1, 6]$ rõ ràng giống như giá trị nhỏ nhất của giá trị nhỏ nhất của đoạn $[1, 4]$ và giá trị nhỏ nhất của đoạn $[3, 6]$.
...
Điều này đòi hỏi chúng ta phải có khả năng tính $\log_2(R - L + 1)$ nhanh.
Bạn có thể thực hiện điều đó bằng cách tính trước tất cả các logarit:
...
for (int i = 2; i <= MAXN; i++)
    lg[i] = lg[i/2] + 1;
```
Ngoài ra, log có thể được tính toán nhanh chóng trong không gian và thời gian không đổi:
```c++
// C++20
#include <bit>
...
```
[Bài kiểm tra hiệu năng này](https://quick-bench.com/q/Zghbdj_TEkmw4XG2nqOpD3tsJ8U) cho thấy việc sử dụng mảng `lg` chậm hơn do lỗi bộ đệm.

Sau đó, chúng ta cần tiền xử lý cấu trúc Bảng thưa. Lần này chúng ta định nghĩa $f$ với $f(x, y) = \min(x, y)$.
...
Độ phức tạp thời gian cho một Truy vấn tìm giá trị nhỏ nhất trên đoạn là $O(1)$.

## Các cấu trúc dữ liệu tương tự hỗ trợ nhiều loại truy vấn hơn

Một trong những điểm yếu chính của cách tiếp cận $O(1)$ được thảo luận trong phần trước là, cách tiếp cận này chỉ hỗ trợ các truy vấn của các [hàm lũy đẳng](https://en.wikipedia.org/wiki/Idempotence).
Tức là, nó hoạt động tốt cho các truy vấn tìm giá trị nhỏ nhất trên đoạn, nhưng không thể trả lời các truy vấn tổng trên đoạn bằng cách tiếp cận này.

Có những cấu trúc dữ liệu tương tự có thể xử lý bất kỳ loại hàm kết hợp nào và trả lời các truy vấn trên đoạn trong $O(1)$.
Một trong số đó được gọi là [Bảng thưa rời rạc](https://discuss.codechef.com/questions/117696/tutorial-disjoint-sparse-table).
Một cái khác sẽ là [Cây căn bậc hai](sqrt-tree.md).
...
* [DevSkill - Multiplication Interval (đã lưu trữ)](http://web.archive.org/web/20200922003506/https://devskill.com/CodingProblems/ViewProblem/19)
